...
```{r}
ggplot(cfb_ei, aes(margin_game, excitement_index)) +
  geom_point()
```


```{r}
bind_rows(mutate(test, type = "Margin + ScoreDiff"),
          mutate(test2, type = "Score_Diff + Home 1st Half"),
          mutate(test3, type = "Score"),
          mutate(test4, type = "4 + Comeback"),
          mutate(test5, type = "5")) %>%
  group_by(type) %>%
